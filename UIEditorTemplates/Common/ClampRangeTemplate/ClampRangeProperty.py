...
class ClampRangeProperty(IWidgetProperties):
    ...
    def __init__(self):
        ...
        self._clamp_widget.minEdit_widget.editingFinished.connect(self.assignMin)
        self._clamp_widget.maxEdit_widget.editingFinished.connect(self.assignMax)
    ...
    def override_default(self, defaults: tuple):
        ...
        self._clamp_widget.minEdit_widget.setText(str(self._min))
        self._clamp_widget.maxEdit_widget.setText(str(self._max))
        self._clamp_widget.minLock_widget.setChecked(self._lockmin)
        self._clamp_widget.maxLock_widget.setChecked(self._lockmax)

        if len(defaults) == 5:
            self._clamp_widget.minEdit_widget.setValidator(self._clamp_widget.validator_type(defaults[4]))
            self._clamp_widget.maxEdit_widget.setValidator(self._clamp_widget.validator_type(defaults[4]))

    def setValue(self, value):
        self._clamp_widget.minEdit_widget.setText(value['minValue'])
        self._clamp_widget.maxEdit_widget.setText(value['maxValue'])
    ...
    def value(self):
        clamp_dict = {}
        clamp_dict['minValue'] = self._clamp_widget.minEdit_widget.text()
        clamp_dict['maxValue'] = self._clamp_widget.maxEdit_widget.text()
    ...
    def lockState(self, button):
        if button is self._clamp_widget.minLock_widget:
            self._lockmin = button.isChecked()
        else:
            self._lockmax = button.isChecked()


    def assignMin(self):
        self._min = int(self._clamp_widget.minEdit_widget.text())
        self.setFocus()


    def assignMax(self):
        self._max = int(self._clamp_widget.maxEdit_widget.text())
        self.setFocus()
