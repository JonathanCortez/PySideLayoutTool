...
class WidgetSetup(QtWidgets.QWidget):
    ...
    def notify_conditions(self):
        self._disable_implementation(
            self._eval_expression(self._disable_str_list) if self._disable_str_list is not None else False)
        self._hidden_implementation(
            self._eval_expression(self._hidden_str_list) if self._hidden_str_list is not None else False)

    def _eval_expression(self, expression) -> bool:
        expressionStr = ''
        expressionStr = ' '.join(
            [str(elem.eval() if isinstance(elem, QtWidgets.QWidget) else elem) for elem in expression])
    ...
    def _close_tab_change(self, index):
        ...
        for index in range(0, self._folder_widget.count()):
            ...
            for key in widget_template.templateGroupData():
                ...
                widget.__property_instances__['name'].setValue(
                    widget.__property_instances__['name'].value()[:-1] + str(index + 1))
                ...
                if widget._label_widget:
                    widget.setToolTip(
                        f"<p style='white-space:pre'> Parameter: <B>{widget.name()}<B></p> {widget.tooltip()} " if widget.tooltip() != '' else f"<p style='white-space:pre'> Parameter: <B>{widget.name()}<B></p>")
                    widget.setStyleSheet("QToolTip { color: #ffffff; background-color: #484848; border: 0px;}")

                    widget._label_widget.setToolTip(
                        f"<p style='white-space:pre'> Parameter: <B>{widget.name()}<B></p> {widget.tooltip()} " if widget.tooltip() != '' else f"<p style='white-space:pre'> Parameter: <B>{widget.name()}<B></p>")
                    widget._label_widget.setStyleSheet(
